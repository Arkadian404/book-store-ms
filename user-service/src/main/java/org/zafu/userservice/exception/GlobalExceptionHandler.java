...
import org.zafu.userservice.dto.ErrorResponse;
...
//                .error(errors.size() > 1 ? String.valueOf(errors) : errors.get(0))
    ...
    ResponseEntity<ErrorResponse> handleDIVE(DataIntegrityViolationException ex, HttpServletRequest request){
        String rootMsg = ex.getMostSpecificCause().getMessage();
        String msg = ex.getMessage();
        log.info("rootMsg {}", rootMsg);
        log.info("msg {}", msg);
        if(rootMsg != null){
