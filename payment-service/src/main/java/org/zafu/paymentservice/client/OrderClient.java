package org.zafu.paymentservice.client;

import org.springframework.cloud.openfeign.FeignClient;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PutMapping;
import org.zafu.paymentservice.config.OrderFeignConfig;
import org.zafu.paymentservice.dto.ApiResponse;
import org.zafu.paymentservice.dto.response.InternalOrderResponse;
import org.zafu.paymentservice.dto.response.OrderResponse;

import java.util.Optional;

@FeignClient(
        name = "order-service",
        url = "${app.client.order-url}",
        configuration = OrderFeignConfig.class
)
public interface OrderClient {
    @GetMapping("/orders/internal/code/{orderCode}")
    Optional<ApiResponse<InternalOrderResponse>> getInternalOrderByCode(@PathVariable String orderCode);
    @PutMapping("/orders/internal/code/{orderCode}")
    Optional<ApiResponse<Void>> markOrderAsPaid(@PathVariable String orderCode);
}
